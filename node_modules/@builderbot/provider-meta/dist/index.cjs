...
const processIncomingMessage = async ({ messageId, messageTimestamp, pushName, message, to, jwtToken, version, numberId, fileData, fromMe, userId, }) => {
    let responseObj;
    switch (message.type) {
        case 'text': {
            responseObj = {
 ...
 */
class MetaCoreVendor extends EventEmitter {
    /**
 ...
 */
async function getProfile(version, numberId, token) {
    const response = await axios.get(`https://graph.facebook.com/${version}/${numberId}`, {
        headers: {
            Authorization: `Bearer ${token}`,
        },
    });
...
class MetaProvider extends bot.ProviderClass {
    constructor(args) {
         ...
         */
        this.fixPrefixMetaNumber = (phoneNumber) => {
         ...
         */
        this.saveBuffer = async (ctx, options = {}) => {
            try {
                const url = ctx?.url ?? ctx?.fileData?.url;
        ...
        this.sendImage = async (to, mediaInput = null, caption, context = null) => {
            ...
            formData.append('file', require$$6.createReadStream(mediaInput), {
        ...
        this.sendFile = async (to, mediaInput = null, caption, context = null) => {
            ...
            formData.append('messaging_product', 'whatsapp');
            const nameOriginal = require$$1$1.basename(mediaInput) || 'Doc';
            const { data: { id: mediaId }, } = await axios.post(`${URL}/${this.globalVendorArgs.version}/${this.globalVendorArgs.numberId}/media`, formData, {
                headers: {
                    Authorization: `Bearer ${this.globalVendorArgs.jwtToken}`,
                    ...formData.getHeaders(),
        ...
        this.sendAudio = async (to, pathVideo = null, context = null) => {
            ...
            if (!mimeType?.includes('ogg') && !mimeType?.includes('opus')) {
                audioPath = await bot.utils.convertAudio(pathVideo, 'ogg');
            }
            const formData = new FormData();
         ...
         */
        this.requestLocation = async (to, bodyText) => {
            to = parseMetaNumber(to);
            const body = {
                messaging_product: 'whatsapp',
                recipient_type: 'individual',
         ...
         */
        this.sendLocationRequest = async (to, bodyText) => {
            to = parseMetaNumber(to);
            const body = {
                messaging_product: 'whatsapp',
                recipient_type: 'individual',
         ...
         */
        this.sendPresenceUpdate = async (messageId) => {
            const body = {
                messaging_product: 'whatsapp',
                status: 'read',
                message_id: messageId,
         ...
         */
        this.typing = async (messageId, ms) => {
